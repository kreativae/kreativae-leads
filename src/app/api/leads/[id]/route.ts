...
export async function DELETE(_req: Request, ctx: Ctx) {
  ...
  if (auth.error) return auth.error;
  const { id } = await ctx.params;
  let deleted: typeof leads.$inferSelect | undefined;
  try {
    [deleted] = await db.delete(leads).where(eq(leads.id, id)).returning();
  } catch (err) {
    // Nunca deixar a exclusao falhar em silencio: sem isto, o front-end
    // trata qualquer erro (incluindo violacao de chave estrangeira) como
    // sucesso e some da tela um lead que continua existindo no banco.
    return NextResponse.json(
      {
        ok: false,
        error:
