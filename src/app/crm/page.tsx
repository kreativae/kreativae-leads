...
export default function CrmPage() {
  ...
  const colunaAtual = selecionado
    ? (columns?.find((c) => c.leads.some((l) => l.id === selecionado.id)) ??
      null)
    : null;
  ...
  return (
    <div className="space-y-5">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          >
            <Pin
              ...
              fill={novosFixado ? "currentColor" : "none"}
            />
          </button>
          <button
            type="button"
      ...
      <div className="flex h-[calc(100vh-15rem)] min-h-[480px] flex-col gap-3">
        {busca && columns && (
          <div className="flex flex-none flex-wrap items-center gap-x-2 gap-y-1 rounded-2xl border border-volt/25 bg-volt/[0.06] px-4 py-3 text-[13px]">
            ...
            <span className="text-zinc-400">
              A busca varre todos os leads, incluindo os novos, e ignora o
              filtro de pesquisa.
            </span>
            <button
              type="button"
        ...
        {selecionado && (
          <LeadDrawer
            ...
            onNext={
              colunaAtual &&
              posicao >= 0 &&
              posicao < colunaAtual.leads.length - 1
                ? () => setSelecionado(colunaAtual.leads[posicao + 1])
                : undefined
            }
          />
